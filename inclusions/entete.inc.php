<?php 
  ...
  $languesDispos = languesDisponibles();
  $lan = determinerLangueSite('fr', $languesDispos);

  // Textes statiques du site
  include("i18n/$lan/textes.php");
?>
  ...
  <div id="conteneur" class="page-<?php echo $page; ?>">
    <header>
      <div class="barre-haut">
        ...
        <?php if($page !== 'accueil') { ?>
          <h1 class="logo"><a href="index.php"><?= $entNomSite ?></a></h1>
        <?php } ?>
        <nav class="i18n">
          <!-- Affichage des langues disponibles -->
          <?php foreach($languesDispos as $codeLangue) { ?>
            <a href="?langue=<?= $codeLangue ?>" class="<?= $lan === $codeLangue ? 'actif' : ''; ?>"  title="Nom de la langue"><?= $codeLangue ?></a>
          <?php } ?>
        </nav>
      </div>
      <div class="titre-page">
        <?php if($page === 'accueil') { ?>
          ...
          <h3><?= $entSousTitre ?></h3>
        <?php } else { ?>
          <h1><?= mb_strtoupper($meta[$page]['h1']); ?></h1>
        <?php } ?>
      </div>
      <?php if($page === 'accueil') { ?>
      ...
      <nav class="nav-principale">
        <!-- Remarquez l'utilisation de l'opérateur ternaire à la place d'une 
        instruction conditionnelle -->
        <a class="<?= ($page==='menu') ? 'actif' : ''; ?>" href="menu.php" title="<?= $entTitreNavMenu ?>">
        ...
        <a class="<?= ($page==='vins') ? 'actif' : ''; ?>" href="vins.php" title="<?= $entTitreNavVins ?>">
        ...
        <a href="javascript: void(0);" title="<?= $entTitreNavReservation ?>">
